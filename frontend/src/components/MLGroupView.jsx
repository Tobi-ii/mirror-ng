    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-600 font-black uppercase tracking-wider">All Transactions Processed</p>
          <p className="text-[10px] text-slate-700 mt-1">Every transaction has been categorized.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="grid grid-cols-3 gap-3 flex-1">
        ...
        <button onClick={openGroupList}
          className="bg-indigo-500/[0.04] border border-indigo-500/15 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center hover:bg-indigo-500/[0.08] active:scale-[0.97] transition-all cursor-pointer h-full">
          <Brain size={24} className="text-indigo-400 mb-3" />
          <span className="text-[9px] text-indigo-400 font-black uppercase tracking-widest mb-1">ML Groups</span>
          <span className="text-2xl font-black tabular-nums text-white">{mlTotalTx}</span>
          <span className="text-[9px] text-slate-600 font-mono mt-1">{fmtK(mlTotalAmt)}</span>
        </button>

        {/* Uncategorized Card */}
        {ungrouped.length > 0 ? (
          <button onClick={() => openDetail('__ungrouped')}
            className="bg-white/[0.02] border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center hover:bg-white/5 active:scale-[0.97] transition-all cursor-pointer h-full">
            <div className="w-6 h-6 rounded-full bg-white/5 flex items-center justify-center mb-3">
              <span className="text-slate-500 text-sm font-black">?</span>
            </div>
            <span className="text-[9px] text-slate-500 font-black uppercase tracking-widest mb-1">Uncategorized</span>
            <span className="text-2xl font-black tabular-nums text-white">{ungrouped.length}</span>
            <span className="text-[9px] text-slate-600 font-mono mt-1">{fmtK(groupTotal(ungrouped))}</span>
          </button>
        ) : (
          <div className="bg-white/[0.01] border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center h-full">
            <CheckCircle2 size={18} className="text-emerald-500/50 mb-3" />
            <span className="text-[9px] text-slate-600 font-black uppercase tracking-widest">All Set</span>
          </div>
        )}
        ...
        {credited.length > 0 ? (
          <button onClick={() => openDetail('__credits')}
            className="bg-emerald-500/[0.03] border border-emerald-500/10 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center hover:bg-emerald-500/[0.06] active:scale-[0.97] transition-all cursor-pointer h-full">
            <ArrowDownLeft size={22} className="text-emerald-400 mb-3" />
            <span className="text-[9px] text-emerald-400 font-black uppercase tracking-widest mb-1">Income</span>
            <span className="text-2xl font-black tabular-nums text-white">{credited.length}</span>
            <span className="text-[9px] text-slate-600 font-mono mt-1">{fmtK(groupTotal(credited))}</span>
          </button>
        ) : (
          <div className="bg-white/[0.01] border border-white/5 rounded-[2rem] p-6 flex flex-col items-center justify-center text-center h-full">
            <CheckCircle2 size={18} className="text-emerald-500/50 mb-3" />
